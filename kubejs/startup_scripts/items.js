onEvent('item.registry', event => {
    ...
    event.create('hyperspace_time_broom_mk1').displayName('超时空扫帚').tooltip('右键清除所有掉落物')
    ...
    event.create('raw_andesite_alloy').displayName('粗制安山合金')
    event.create('gel').displayName('凝胶')

    event.create('machinery_alloy').displayName('机械合金')

    event.create('mysterious_amulet').displayName('神秘的护身符').glow(true).rarity(RARITY_UNCOMMON).tooltip('§b右键获得游戏阶段“mysterious_asylum”')
    event.create('ender_link').displayName('末影的链接').glow(true).rarity(RARITY_UNCOMMON).tooltip('§b右键获得游戏阶段“ender_link”')

    event.create('twilight_gemstone').displayName('暮光宝石').glow(true).rarity(RARITY_UNCOMMON).tooltip('§b用于激活暮色森林传送门')
    ...
    event.create('item_value_conversion_core').displayName('物价转换核心').rarity(RARITY_RARE)
