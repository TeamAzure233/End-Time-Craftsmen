onEvent('player.logged_in', event => {
    ...
    if (!player.stages.has('starting_items')) {
      ...
      //server.runCommandSilent(`op Gofishfly233333`)//才没有那样想呢
    }
...
onEvent('item.right_click', event => {
    if (event.item.id == 'kubejs:mysterious_amulet') {
        event.item.count--;
        event.server.runCommandSilent(`gamestage add "${event.player.name}" mysterious_asylum`)
    }
})
onEvent('item.right_click', event => {
    if (event.item.id == 'kubejs:ender_link') {
        event.item.count--;
        event.server.runCommandSilent(`gamestage add "${event.player.name}" ender_link`)
    }
})
onEvent('item.right_click', event => {
